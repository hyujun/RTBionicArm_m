...
void SerialManipulator::VelocityInvConvert( int32_t *_enc_sec, double *_rad_sec )
{
	for(int i=0; i < this->mDoF_Total; i++)
	{
		_enc_sec[i] = (int)(_rad_sec[i]/(2*M_PI)*(serial_Motor_info[i].motor_harmonic*serial_Motor_info[i].enc_size));
	}
	return;
}
